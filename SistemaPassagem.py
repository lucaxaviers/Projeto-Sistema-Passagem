...
def cadastrar_voo():
    ...
    for i in range(escalas):
        cidade_escala = input(f"Escala {i + 1}: ").lower()
        escalas_cidades.append(cidade_escala)

    voos[codigovoo] = {"origem": origem,"destino": destino,"escalas": escalas,"preco": preco,"lugares_disponiveis": lugaresdisp,"escalas_cidades": escalas_cidades}
...
def voo_com_menos_escalas():
    ...
    for codigo, voo in voos.items():
        if voo["origem"].lower() == origem and voo["destino"].lower() == destino:
